...
export async function GET(
  ...
  try {
    ...
    if (!backendUrl) {
      console.error('[USER-DETAIL-ROUTE] NEXT_PUBLIC_API_BASE_URL not configured');
      return NextResponse.json(
    ...
    const backendEndpoint = `${backendUrl}/api/v1/admin/users/${id}`;
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[USER-DETAIL-ROUTE] Backend request failed:', {
    ...
    console.error('[USER-DETAIL-ROUTE] Error:', error);
...
export async function PUT(
  ...
  try {
    ...
    const backendUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
    if (!backendUrl) {
      console.error('[USER-UPDATE-ROUTE] NEXT_PUBLIC_API_BASE_URL not configured');
      return NextResponse.json(
        { error: 'Server configuration error' },
        { status: 500 }
      );
    ...
    if (!backendResponse.ok) {
      const errorData = await backendResponse.json().catch(() => ({}));
      console.error('[USER-UPDATE-ROUTE] Backend request failed:', {
        status: backendResponse.status,
        userId: id,
        error: errorData,
      });
...
export async function DELETE(
  ...
  try {
    ...
    const backendUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
    if (!backendUrl) {
      console.error('[USER-DELETE-ROUTE] NEXT_PUBLIC_API_BASE_URL not configured');
      return NextResponse.json(
        { error: 'Server configuration error' },
        { status: 500 }
      );
    ...
    if (!backendResponse.ok) {
      const errorData = await backendResponse.json().catch(() => ({}));
      console.error('[USER-DELETE-ROUTE] Backend request failed:', {
        status: backendResponse.status,
        userId: id,
        error: errorData,
      });
