 ...
 * 
 * Proxies requests to the backend Go API for updating user status.
 * Requires authentication via httpOnly session cookie.
 * 
 * @module app/api/admin/users/[id]/status
 */
...
export async function PUT(
  ...
  try {
    ...
    if (!backendUrl) {
      console.error('[USER-STATUS-ROUTE] NEXT_PUBLIC_API_BASE_URL not configured');
      return NextResponse.json(
    ...
    const backendEndpoint = `${backendUrl}/api/v1/admin/users/${id}/status`;
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[USER-STATUS-ROUTE] Backend request failed:', {
