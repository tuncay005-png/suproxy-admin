 ...
 * 
 * Proxies requests to the backend Go API for updating user role.
 * Requires authentication via httpOnly session cookie.
 * 
 * @module app/api/admin/users/[id]/role
 */
...
export async function PUT(
  ...
  try {
    ...
    if (!backendUrl) {
      console.error('[USER-ROLE-ROUTE] NEXT_PUBLIC_API_BASE_URL not configured');
      return NextResponse.json(
    ...
    const backendEndpoint = `${backendUrl}/api/v1/admin/users/${id}/role`;
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[USER-ROLE-ROUTE] Backend request failed:', {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Failed to update user role' },
        { status: backendResponse.status }
      );
    }
