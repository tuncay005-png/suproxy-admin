...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!backendUrl) {
      console.error('[USERS-ROUTE] NEXT_PUBLIC_API_BASE_URL not configured');
      return NextResponse.json(
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[USERS-ROUTE] Backend request failed:', errorData);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Forward to backend admin user creation endpoint
    const backendEndpoint = `${backendUrl}/api/v1/admin/users`;
