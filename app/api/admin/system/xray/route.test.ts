/**
 * Unit Tests for System Xray API Route
 * 
 * Tests the proxy route for Xray system status endpoint.
 */
...
describe('GET /api/admin/system/xray', () => {
  ...
  it('should return 401 when session token is missing', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/admin/system/xray');
  ...
  it('should successfully proxy request to backend', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      `${mockBackendUrl}/api/v1/admin/system/xray`,
    ...
    expect(data).toEqual(mockXrayData);
  });

  it('should handle backend errors gracefully', async () => {
    const mockErrorResponse = {
      message: 'Failed to fetch Xray system status',
    };
    ...
    expect(response.status).toBe(500);
    expect(data.error).toBe('Failed to fetch Xray system status');
  });
