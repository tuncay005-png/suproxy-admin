 ...
 * 
 * Proxies requests to the backend Go API for system health check.
...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendEndpoint = `${backendUrl}/api/v1/admin/system/health`;
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[HEALTH-ROUTE] Backend request failed:', errorData);
      
      return NextResponse.json(
        { error: errorData.message || 'Failed to fetch system health' },
