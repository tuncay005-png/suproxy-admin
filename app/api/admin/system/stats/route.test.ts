/**
 * Unit Tests for System Stats API Route
 * 
 * Tests the proxy route for system statistics endpoint.
 */
...
describe('GET /api/admin/system/stats', () => {
  ...
  it('should return 401 when session token is missing', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/admin/system/stats');
  ...
  it('should successfully proxy request to backend', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      `${mockBackendUrl}/api/v1/admin/system/stats`,
  ...
  it('should handle backend errors gracefully', async () => {
    ...
    expect(response.status).toBe(500);
    expect(data.error).toBe('Failed to compute statistics');
  });
