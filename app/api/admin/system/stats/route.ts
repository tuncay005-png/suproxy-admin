 ...
 * 
 * Proxies requests to the backend Go API for system statistics.
 * Requires authentication via httpOnly session cookie.
 * 
 * @module app/api/admin/system/stats
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendEndpoint = `${backendUrl}/api/v1/admin/system/stats`;
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[STATS-ROUTE] Backend request failed:', errorData);
      
      return NextResponse.json(
        { error: errorData.message || 'Failed to fetch system stats' },
