/**
 * Unit Tests for System Version API Route
 * 
 * Tests the proxy route for API version information endpoint.
 */
...
describe('GET /api/admin/system/version', () => {
  ...
  it('should return 401 when session token is missing', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/admin/system/version');
  ...
  it('should successfully proxy request to backend', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      `${mockBackendUrl}/api/v1/admin/system/version`,
    ...
    expect(data).toEqual(mockVersionData);
  });
  ...
  it('should handle backend errors gracefully', async () => {
    ...
    expect(response.status).toBe(500);
    expect(data.error).toBe('Version information unavailable');
  });
