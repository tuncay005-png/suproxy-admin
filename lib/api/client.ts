...
class ApiClient {
  ...
  private async request<T>(endpoint: string, options?: RequestInit, _isRetryAfterRefresh = false): Promise<T> {
    let url: string;
    ...
    try {
      ...
      // TOKEN_EXPIRED handling for BOTH server and client
      if (!response.ok && response.status === 401 && !endpoint.includes('/api/auth/refresh') && !_isRetryAfterRefresh) {
        ...
        try {
          ...
          if (errorCode === 'TOKEN_EXPIRED' || errorMessage.includes('token has expired') || errorMessage.includes('access token has expired')) {
            ...
            console.log(`[API-CLIENT] TOKEN_EXPIRED detected (${isServerSide ? 'server-side' : 'client-side'}), attempting refresh...`);
          } else {
        ...
        if (shouldRetryWithRefresh) {
          ...
          if (refreshSuccess) {
            console.log(`[API-CLIENT] Retrying original request after successful refresh (${isServerSide ? 'server' : 'client'})`);
